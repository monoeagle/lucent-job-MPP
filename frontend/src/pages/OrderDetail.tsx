...
export default function OrderDetail() {
  ...
  return (
    <div>
      ...
      {isEditable && !order.business_reason ? (
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Geschaeftsgrund <span className="text-red-500">*</span>
            <span className="text-xs text-gray-400 ml-1">(erforderlich zum Einreichen)</span>
          </label>
          <input
            type="text"
            placeholder="Grund fuer diese Bestellung..."
            className="w-full px-3 py-2 border border-red-400 bg-red-50 rounded-md text-sm"
            onBlur={async (e) => {
              ...
              if (reason && orderId) {
                ...
                if (token) {
                  await ordersApi.updateOrder(token, orderId, { business_reason: reason })
