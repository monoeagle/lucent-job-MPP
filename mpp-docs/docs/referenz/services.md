...
**Parameter-Validierung (Reihenfolge):**
...
- `submit_order(order_id)` — Bestellung einreichen (Transition: validated → submitted)
...
- `resolve_context(location_id, tenant_id, security_zone_id, network_id, user_id)` — Loest Kontext aus CMDB auf und validiert
...
Protokolliert alle relevanten Aktionen mit Actor, Aktion, Entitaet und Details. Bietet Filter- und Export-Funktionen fuer das Admin-Dashboard.

---

## NotificationService

Erzeugt Benachrichtigungen bei Status-Aenderungen (Order submitted, approved, rejected, done, failed). Fire-and-Forget-Architektur mit E-Mail-Stub in der Entwicklung.
