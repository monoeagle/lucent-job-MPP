...
>
> Dieses Repo ist in mehreren Punkten **noch nicht** vollständig produktionsreif.
> Die Anleitung beschreibt das **Zielbild** und markiert jede offene Stelle mit einer
> **„⚠ Lücke"-Box**. Die wichtigsten Punkte vorab:
>
> 1. **Kein WSGI-Server in den Dependencies.** `requirements.txt` enthält **kein**
>    `gunicorn`. Start im Repo erfolgt überall via `flask run` (Dev-Server). Diese
>    Anleitung installiert gunicorn zusätzlich (Schritt 6/10).
> 2. **Kein lauffähiger Produktiv-Auth.** `AUTH_MODE=ldap` wirft beim Login
...
> 3. **`CMDB_MODE=live` ist nicht implementiert**, es gibt keinen Live-CMDB-Client.
> 4. **Kein `ProxyFix`/CORS** im Backend — funktioniert nur same-origin hinter nginx.
> 5. **`alembic.ini` ignoriert `DATABASE_URL`** (Host hartkodiert) — Schritt 8.
>
...
# WSGI-Server zusätzlich installieren (siehe Lücke unten)
/opt/mpp/venv/bin/pip install gunicorn

# zurück zum Admin
exit
```

> ### ⚠ Lücke: gunicorn ist kein Projekt-Dependency
> `requirements.txt` listet `flask, sqlalchemy, alembic, psycopg2-binary, pyjwt,
> python-dotenv, pyyaml, requests` — **kein** gunicorn/uwsgi/waitress. Wir
> installieren gunicorn hier manuell. **Empfehlung:** `gunicorn` dauerhaft in
> `requirements.txt` (oder ein `requirements/production.txt`) aufnehmen, damit der
> Re-Deploy reproduzierbar bleibt.
...
# Schema auf den aktuellen Stand bringen
$PY -m alembic upgrade head
```

> ### ⚠ Lücke: `alembic.ini` liest `DATABASE_URL` NICHT
> In `alembic.ini` ist `sqlalchemy.url = postgresql://mpp:mpp@localhost:5432/mpp_dev`
> **hartkodiert** (`migrations/env.py` nutzt `config.get_main_option("sqlalchemy.url")`),
> d. h. Alembic ignoriert die Umgebungsvariable. Für die Produktions-DB vor dem
> `upgrade` die Zeile anpassen:
> ```bash
> sudo -iu mpp sed -i \
>   "s#^sqlalchemy.url = .*#sqlalchemy.url = postgresql://mpp:<DB-PASSWORT>@127.0.0.1:5432/mpp_prod#" \
>   /opt/mpp/app/alembic.ini
> ```
> **Besser** (Empfehlung): `migrations/env.py` so ändern, dass es
> `os.environ["DATABASE_URL"]` bevorzugt — dann entfällt das Editieren bei jedem Deploy.
...
> ### ⚠ Lücke: kein `ProxyFix`
> Die App wertet `X-Forwarded-*`-Header **nicht** aus (kein
> `werkzeug.middleware.proxy_fix.ProxyFix`, keine Trusted-Hosts). Für korrekte
> Client-IPs/Schema-Erkennung hinter nginx empfiehlt sich, in `create_app`
> `app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1)` zu ergänzen. Solange
> die App nur same-origin hinter nginx läuft, ist das für die Funktion unkritisch.
...
# Health-Endpoint (lokal, direkt am gunicorn)
...
curl -s  https://mpp.example.com/api/v1/health    # -> {"status":"ok",...}
```

Erwartung: `http://` leitet nach `https://` um, `/` liefert die SPA, der
Health-Endpoint antwortet mit `{"status":"ok"}`.

> ### ⚠ Lücke: Health prüft die DB nicht
> `GET /api/v1/health` gibt **immer** `200 ok` zurück, solange der Prozess lebt —
> es ist **kein** DB-Check enthalten. Für echtes Monitoring zusätzlich die
> DB-Erreichbarkeit prüfen (siehe Abschnitt 16) oder einen Readiness-Endpoint
> mit DB-Ping ergänzen.
...
# Backend-Deps
/opt/mpp/venv/bin/pip install -r requirements.txt
/opt/mpp/venv/bin/pip install gunicorn        # solange nicht in requirements.txt
...
| App-Health | `curl -s http://127.0.0.1:8001/api/v1/health` |
...
werden sollten (alle aus dem Code-Stand 2026-06-19):

- [ ] **WSGI-Server**: `gunicorn` (o. ä.) in `requirements.txt`/`requirements/production.txt` aufnehmen — aktuell nur `flask run` (Dev-Server) im Repo.
- [ ] **Auth**: `AUTH_MODE=ldap` implementieren (`auth_service.py` → `NotImplementedError`) oder OIDC ergänzen. Ohne das ist in `ENV=production` kein Login möglich.
...
- [ ] **Alembic**: `migrations/env.py` auf `DATABASE_URL` umstellen, damit `alembic.ini` den Host nicht hartkodiert.
- [ ] **ProxyFix**: `ProxyFix` in `create_app` ergänzen für korrekte `X-Forwarded-*`-Auswertung.
- [ ] **Health/Readiness**: DB-Check in den Health-Endpoint (oder separater Readiness-Pfad).
- [ ] **HSTS/SSL-Redirect**: aktuell rein nginx-seitig — bewusst konfigurieren.
